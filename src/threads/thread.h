...
#include <list.h>
#include <stdint.h>
#include <hash.h>       // SOS Implementation project 3
...
struct thread
  {
    ...
    // Semaphores for parent to wait child's exit and load
    struct semaphore child_load;
    struct semaphore child_exit;
   
    // Data structures for Child-Parent Relationship
    struct thread *parent;
    struct list child_list;               
    struct list_elem child_elem;
   
    // File Descriptor Table Elements
    struct file **fd_table;
    int fd;

    // File data structure for executable
...
bool cmp_time_to_wake (struct list_elem *, struct list_elem *, void *);
bool cmp_thread_priority (const struct list_elem *, const struct list_elem *, void *);
void insert_to_alarmCall_list (int64_t);

void multiple_priority_donation (struct thread *);
void nested_priority_donation (struct thread *);
