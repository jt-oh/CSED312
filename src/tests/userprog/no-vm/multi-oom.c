...
consume_some_resources_and_die (int seed)
{
  ...
  volatile int *PHYS_BASE = (volatile int *)0xC0000000;
	...
	//printf("die crash child at depth %d!\n", seed);
  ...
  for (i = 0; i < howmany; i++)
    {
      ...
      if (n > EXPECTED_DEPTH_TO_PASS/2)
        {
					//printf("spawn crash child at depth %d!\n", n);
          child_pid = spawn_child (n + 1, CRASH);
					//printf("crash child pid %d\n", child_pid);
          if (child_pid != -1)
            {
							exit = wait(child_pid);
							//printf("crash child return %d at depth %d\n", exit, n);
      ...
      /* Now spawn the child that will recurse. */
			//printf("spawn rescurse child at depth %d!\n", n);
      child_pid = spawn_child (n + 1, RECURSE);


			//printf("after spawn recurse child\n");
      ...
      int reached_depth = wait (child_pid);
			...
			//printf("after wait recurse child depth %d!\n", n + 1);
